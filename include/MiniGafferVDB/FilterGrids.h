...
namespace MiniGafferVDB
{
	...
	class MINIGAFFERVDB_API FilterGrids : public GafferScene::SceneElementProcessor
	{

	public :

		FilterGrids(const std::string &name = defaultName<FilterGrids>() );
		~FilterGrids();

		IE_CORE_DECLARERUNTIMETYPEDEXTENSION( MiniGafferVDB::FilterGrids, TypeId::FilterGridsTypeId, GafferScene::SceneElementProcessor );
		...
		Gaffer::IntPlug *widthPlug();
		const Gaffer::IntPlug *widthPlug() const;
	...
	IE_CORE_DECLAREPTR( FilterGrids )
